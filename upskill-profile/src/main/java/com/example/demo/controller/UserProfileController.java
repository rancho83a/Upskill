...
//    @PostMapping(value = "/owner",  produces = "application/json")
    ...
    public ResponseEntity<?> editUserProfile(
            @ModelAttribute @Valid UserProfileEditBindingModel userProfile,
        ...
        UserProfileView editedUser = this.userProfileService.editUserProfile(userEmail, userProfile);
...
//        return ResponseEntity.ok(isCreated);
    ...
    public ResponseEntity<?> getAllEmployeesByBusinessOwner(HttpServletRequest request){
        List<EmployeeBindingModel> employeesModel = this.userProfileService.getAllEmployeesByBusinessOwner(request);
